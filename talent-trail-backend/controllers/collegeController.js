...
const getDashboard = async (req, res, next) => {
  ...
  try {
    ...
    const foundSelectedStudents = await Student.find({
      college: id,
      jobsSelected: { $gt: 0 },
    }).exec();
...
const getCompany = async (req, res, next) => {
  ...
  try {
    const foundRecruiter = await Recruiter.findById(recruiterId)
      ...
      .exec();
    if (!foundRecruiter)
      return res.status(404).json({ message: "Recruiter details not found" });
...
const getCourses = async (req, res, next) => {
  const { id } = req;
  try {
    const foundCourses = await College.findById(id)
      .populate("programs")
      .select("programs")
      .exec();
  ...
  try {
    ...
    const foundStudents = await Student.find({ college: id })
      .populate({
        path: "academic",
        match: {
          "currentEducation.course": foundCourse.name,
          "currentEducation.major": foundCourse.specialization,
        },
      })
      .populate("personal")
      .populate("contact")
...
const getDrives = async (req, res, next) => {
  ...
  try {
    ...
    const foundApprovedJobs = await Job.find({
      ...
      .select("jobRole description companyName")
      .exec();
...
const postInstitution = async (req, res, next) => {
  ...
  if (!name || !website || !address || !mobile)
  ...
  try {
    ...
    if (foundInstitution)
      return res
        .status(400)
        .json({ message: "Institution info already exists" });
...
const postPrincipal = async (req, res, next) => {
  ...
  if (!fullName || !position || !email || !mobile)
    return res.status(400).json({ message: "All fields required" });
  ...
  try {
    ...
    if (foundPrincipal)
      return res.status(400).json({ message: "Principal info already exists" });
...
const postPlacement = async (req, res, next) => {
  ...
  try {
    ...
    if (foundPlacement)
      return res.status(400).json({ message: "Placement info already exists" });
...
const postCourse = async (req, res, next) => {
  ...
  if (!name || !duration || !specialization)
    return res.status(400).json({ message: "All fields required" });
...
const putInstitution = async (req, res, next) => {
  ...
  if (!name || !website || !address || !mobile || !institutionId)
    return res.status(400).json({ message: "All fields required" });
  ...
  try {
    ...
    if (!foundInstitution)
      return res.status(400).json({ message: "Institution details not found" });

    if (foundInstitution._id.toString() !== foundCollege.institution.toString())
...
const putPrincipal = async (req, res, next) => {
  ...
  if (!fullName || !position || !email || !mobile || !principalId)
    return res.status(400).json({ message: "All fields required" });
  ...
  try {
    ...
    if (!foundPrincipal)
      return res.status(400).json({ message: "Principal details not found" });

    if (foundPrincipal._id.toString() !== foundCollege.principal.toString())
      return res.status(401).json({ message: "unauthorized" });
...
const putPlacement = async (req, res, next) => {
  ...
  if (!fullName || !position || !email || !mobile || !placementId)
    return res.status(400).json({ message: "All fields required" });
  ...
  try {
    ...
    if (!foundPlacement)
      return res.status(400).json({ message: "Placement details not found" });

    if (foundPlacement._id.toString() !== foundCollege.placement.toString())
      return res.status(401).json({ message: "unauthorized" });
...
const putCourse = async (req, res, next) => {
  ...
  if (!name || !duration || !specialization || !courseId)
    return res.status(400).json({ message: "All fields required" });
  ...
  try {
    ...
    if (!foundCollege.programs.includes(courseId))
      return res.status(401).json({ message: "unauthorized" });
