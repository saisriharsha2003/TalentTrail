...
const getDashboard = async (req, res, next) => {
  ...
  try {
    const foundStudent = await Student.findOne({ username: user })
      ...
      .populate("personal")
      .select(
        "jobsSelected jobsApplied jobsRejected profile username academic personal",
      )
      .lean();
...
const getStudent = async (req, res, next) => {
  ...
  try {
    const foundStudent = await Student.findById(id)
      ...
      .select(
        "username personal contact academic workExperiences projects certifications resume profile rollNo skills interests",
      )
      .exec();
...
const buildStudentPayload = (student) => {
  const totalExperienceMonths = student.workExperiences.reduce((acc, exp) => {
  ...
  return {
    ...
    summary: `
      ${student.workExperiences.map(e => e.role).join(", ")}
      ${student.projects.map(p => p.name).join(", ")}
    `,
    experience: student.workExperiences.map(exp => {
      const start = new Date(exp.startDate);
      const end = exp.endDate === "Present" ? new Date() : new Date(exp.endDate);
      ...
      return { duration_months: Math.max(months, 0) };
    }),
    full_text: `
      ${student.skills.join(", ")}
      ${student.workExperiences.map(e => e.description).join(" ")}
      ${student.projects.map(p => p.description).join(" ")}
    `
  };
};
...
const buildJobPayload = (job) => {
  return {
    ...
    required_experience_months: parseInt(job.experienceRequired) * 12 || 0,
    keywords: job.skills || [],
    critical_skills: job.skills.slice(0, 5)
  };
};
...
const capabilityCal = async (req, res, next) => {
  try {
    ...
    const payload = {
      resume: buildStudentPayload(student),
      job: buildJobPayload(job)
    };

    const response = await axios.post(
      process.env.CAPABILITY_SCORE_API_URL,
      payload
    );
...
const postSkills = async (req, res, next) => {
  ...
  try {
    ...
    if (!Array.isArray(skills) || !Array.isArray(interests)) {
      return res
        .status(400)
        .json({ message: "Skills & interests must be arrays" });
    }
...
const postContact = async (req, res, next) => {
  const { email, collegeEmail, mobile, currentAddress, permanentAddress } =
    req.body;
  if (
    !email ||
    !collegeEmail ||
  ...
  try {
    ...
    if (duplicateUserMail.length > 0)
      return res
        .status(409)
        .json({ message: "User already exist with that mail" });

    const duplicateUserMobile = await Contact.find({ mobile }).exec();
    if (duplicateUserMobile.length > 0)
      return res
        .status(409)
        .json({ message: "User already exist with that mobile number" });
...
const postResume = async (req, res, next) => {
  ...
  try {
    ...
    if (!req.file)
      return res
        .status(400)
        .json({ message: "Only pdf are allowed which are lessthan 2 mb" });
...
const postWork = async (req, res, next) => {
  ...
  if (!organization || !role || !description || !startDate || !endDate)
    return res.status(400).json({ message: "All fields required" });
...
const putAcademic = async (req, res, next) => {
  ...
  if (!currentEducation || !previousEducation || !rollNo || !academicId)
    return res.status(400).json({ message: "All fields required" });
  ...
  try {
    ...
    if (!foundAcademic)
      return res.status(400).json({ message: "Academic details not found" });

    if (foundAcademic._id.toString() !== foundStudent.academic.toString())
      return res.status(401).json({ message: "unauthorized" });
...
const putCertification = async (req, res, next) => {
  ...
  if (!name || !organization || !certificationId)
    return res.status(400).json({ message: "All fields required" });
  ...
  try {
    ...
    const foundCertification =
      await Certification.findById(certificationId).exec();
    if (!foundCertification)
      return res
        .status(400)
        .json({ message: "Certification details not found" });

    if (!foundStudent.certifications.includes(certificationId))
      return res.status(401).json({ message: "unauthorized" });
...
const putContact = async (req, res, next) => {
  ...
  try {
    ...
    const duplicateUserMail = await Contact.find({ email }).exec();
    if (duplicateUserMail.length > 0 && duplicateUserMail.length !== 1)
      return res
        .status(409)
        .json({ message: "User already exist with that mail" });

    const duplicateUserMobile = await Contact.find({ mobile }).exec();
    if (duplicateUserMobile.length > 0 && duplicateUserMobile.length !== 1)
      return res
        .status(409)
        .json({ message: "User already exist with that mobile number" });
    ...
    if (!foundContact)
      return res.status(400).json({ message: "Contact details not found" });

    if (foundContact._id.toString() !== foundStudent.contact.toString())
      return res.status(401).json({ message: "unauthorized" });
...
const putPersonal = async (req, res, next) => {
  const { fullName, fatherName, motherName, dateOfBirth, gender, personalId } =
    req.body;
  if (
    !fullName ||
    !fatherName ||
  ...
  try {
    ...
    if (foundPersonal._id.toString() !== foundStudent.personal.toString())
      return res.status(401).json({ message: "unauthorized" });
...
const deleteProject = async (req, res, next) => {
  ...
  try {
    ...
    if (!foundStudent.projects.includes(pId))
      return res.status(401).json({ message: "unauthorized" });
