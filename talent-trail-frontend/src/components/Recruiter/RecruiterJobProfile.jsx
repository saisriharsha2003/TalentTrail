...
const RecruiterJobProfile = () => {
  ...
  useEffect(() => {
    const fetchJobProfile = async () => {
      try {
        const res = await axios.get("/recruiter/jobProfile/" + id);
        setJob(res?.data?.job || {});
    ...
    <div className="container my-5" style={{ maxWidth: "1200px" }}>
      ...
      <div className="card shadow-sm border-0 rounded-4 p-4 mb-4">
        ...
        <div className="d-flex flex-wrap gap-3 mt-3">
          <span className="badge bg-light text-dark">📍 {job.location || "N/A"}</span>
          <span className="badge bg-light text-dark">💼 {job.employmentType || "N/A"}</span>
          <span className="badge bg-light text-dark">🧠 {job.experienceRequired || "N/A"}</span>
          {job.salaryRange && (
        ...
        <div className="col-md-7">
          <div className="card shadow-sm border-0 rounded-4 p-4">
            ...
            <p>
              ...
              {job.eligibleBatch || "Not specified"}
            </p>
